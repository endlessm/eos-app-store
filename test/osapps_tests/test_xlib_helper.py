...
class TestXlibHelper(unittest.TestCase):
    def setUp(self):
        ...
        mock_display_provider = Mock()
        mock_display_provider.display = Mock(return_value=self._mock_display)
        
        self._test_object = XlibHelper(mock_display_provider, self._mock_logger)
    ...
    def test_bring_app_to_front_if_app_is_open(self):
        mock_get_application_key = Mock(return_value="firefox")
        mock_get_all_tasks = Mock(return_value=["", ""])

        app_key = "firefox"

        self._test_object.get_application_key = mock_get_application_key    
        self._test_object._get_all_tasks = mock_get_all_tasks
        
        return_value = self._test_object.bring_app_to_front(app_key)
        
        self.assertTrue(return_value)

    def test_return_false_if_given_app_is_not_running(self):
        mock_get_application_key = Mock(return_value="firefox")
        mock_get_all_tasks = Mock(return_value=["", ""])
